...
Game-start throttling is separate from spending. A session can burst six starts and replenishes one every 30 seconds; a normalized client address can burst 30 and replenishes one every three seconds to accommodate shared networks. Invitations charge the host when created and the guest when joined; public matching and friend rematches check both sessions atomically. Simulator starts have a separate burst of 20, replenishing every five seconds. These short-lived in-memory buckets do not survive restarts and are not protection against a distributed attacker. OpenRouter remains the hard spending boundary.

All model attempts, including simulator judging, retries and worker hedges, share at most 40 active requests, with at most two per game and 80 waiting attempts. Waiting is included in the request timeout and canceled when the game closes. No token or dollar counters are maintained.

Provider availability is refreshed outside the game authority; admission checks the cached status synchronously. Score reads are coalesced and bounded, and result writes run as background work while the room stays in saving. Their completion re-enters the authority and verifies the room/state before publishing. No database or provider wait occupies the global state queue. Each socket can queue at most eight actions and timer ticks coalesce rather than accumulating. Startup still verifies dependencies before serving, and shutdown waits for bounded outstanding outcome work.
