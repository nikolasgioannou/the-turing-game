...
Game-limit notices explain that players should return later, or tomorrow for a confirmed daily key limit. Interrupted rounds never count as a win or loss. Matchmaking pauses on unavailable status; recovery shows a dismissible notification and requires an explicit play action. Friend rematch context is preserved. Confirmed scheduled key exhaustion may show the expected reset in the player’s local time; unknown credit failures never promise a reset.

At the concurrent-game limit, public matchmaking shows “Waiting for a game slot” and automatically matches compatible players when space opens. Players can cancel. New friend invitations and rematches ask players to retry when full; already-created invitations reserve space. Existing games continue normally. The default is 20 unfinished rooms, configurable by the operator.

Before joining, role selection and name entry expose game-data information; the lobby keeps the same information accessible. It explains messages, names, unsent human drafts and device/time hints sent through OpenRouter to Anthropic, and distinguishes application storage from provider policies. No composer helper text is added. The public score is labeled “Games the bot won” and explicitly includes repeat players and friend rounds; it is not a unique-person or scientific measure.
