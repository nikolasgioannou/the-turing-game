...
## Readiness, connection behavior and repository checks

Added a bounded, cached and coalesced /api/ready probe for PostgreSQL and game-loop responsiveness. Provider exhaustion/capacity is reported separately from infrastructure readiness. Deployment checks now have hard request/retry limits. No external monitoring or alert destination was configured; broader telemetry remains deferred. Added pinned read-only push/PR validation and weekly Bun/Actions Dependabot configuration. Enabled repository vulnerability alerts; direct-push permissions remain unchanged.

Lobby snapshots now publish only changed per-peer data. A controlled 100-visitor test emitted zero repeat lobby events across 20 unchanged ticks. WebSocket backpressure is bounded, admission counts live sockets, and the session endpoint exposes connection-limit retry guidance. Browser retries now use jitter, cooldowns and a handshake timeout. A real local Chrome check verified rejection, guidance, recovery and server disconnect; it caught and fixed a remaining lobby message that incorrectly promised match restoration. No model calls or games were created. Existing 20-game admission/queue behavior was reviewed and retained; full friend creation/rematches use retry guidance rather than a separate queue.

The simulator is now local-only, with header authorization, bounded state polling and tab-scoped credentials. Production never mounts it, so operator runs cannot compete for production capacity/credit. Added role/name-entry/lobby information about model data sharing and honest aggregate-score wording. Both Chromium and WebKit passed all 50 review states plus narrow/landscape/desktop and keyboard viewport checks; physical mobile keyboards remain untested. Full validation passed 254 tests, types and build. Real HTTP checks passed readiness, unavailable-provider admission, simulator authorization and rate-limit guidance. Temporary servers were stopped.

Completed eight tickets. Asset provenance and targeted bot evaluation plans are documented, but those two tickets remain open pending ownership evidence and bounded provider evaluation approval. No prompt changes, paid model evaluations, new machines or deployment were performed in this batch. Repository workflow files take effect after push; only the vulnerability-alert setting changed remotely during implementation.

## Verified readiness and connection deployment

Deployed 28f660b through successful production workflow 35492044963. Formatting, types, tests, build and the bounded readiness check passed in GitHub Actions. Public /api/ready returned ok=true and acceptingGames=true; the authenticated lobby WebSocket connected successfully. The production simulator dashboard and API both return 404 as intended, and the single Fly machine's liveness check passes. Verification created no games or model completions. This supersedes the prior batch's not-deployed status.
