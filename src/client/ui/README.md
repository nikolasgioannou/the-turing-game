...
Project-owned React components built with Tailwind v4. Import from `./ui` in client modules. Native
...
Use explicit submit buttons inside forms. Supply visible field labels or `aria-label`. Selection
state belongs in `aria-pressed`, not an independent styling flag. Use a link for navigation and a
button for actions. Keep style variants as static class strings so Tailwind detects them.

Use `className` for surrounding layout. Add a named component variant when changing its appearance
or sizing; do not pile conflicting utilities onto a component. Add new shared styling here instead
of route-specific CSS overrides. Theme tokens, font loading, global defaults, reduced-motion rules
and the custom arcade title/cabinet effects live in `../styles.css`.
